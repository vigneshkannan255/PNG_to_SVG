...
def main():
    print("[+] PNG to SVG conversion started")
    png_path = os.getcwd()+"/png"
    png_list = os.listdir(png_path)
    os.makedirs(os.getcwd()+'/svg_images/', exist_ok=True)
    for img in png_list:
    	image = Image.open("png/"+img).convert('RGBA')
    	svg_image = rgba_image_to_svg_contiguous(image)
    	#svg_image = rgba_image_to_svg_pixels(image)
    	with open("svg_images/"+img+".svg", "w") as text_file:
        	text_file.write(svg_image)
        	print("[+] PNG to SVG completed for " + img)
